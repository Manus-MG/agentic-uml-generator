  ...
  if (data === null || data === undefined) {
    return (
      <div className="flex items-center gap-2 py-0.5 text-xs font-mono">
        <span className="text-accent-orange">{title}:</span>
  ...
  return (
    <div className="py-0.5">
      <button
        ...
        className="flex w-full items-center gap-1.5 rounded-sm px-1 py-0.5 text-left text-xs font-mono text-text-secondary hover:bg-bg-tertiary/60"
      >
        {open ? <CaretDown size={12} weight="bold" /> : <CaretRight size={12} weight="bold" />}
        <span className="font-semibold text-text-primary">{title}</span>
        <span className="rounded-sm bg-bg-tertiary px-1 py-0.5 text-[10px] text-text-muted">
          {isArray ? `[${count}]` : `{${count}}`}
        </span>
      </button>
...
export function ModelDrawer({
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center gap-2 p-8 text-center text-xs text-text-muted">
        <Cpu size={20} className="animate-spin text-accent-orange" />
        <span>Loading canonical system model…</span>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="rounded-sm border border-accent-rose/30 bg-accent-rose/10 p-4 text-xs text-accent-rose">
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-1 rounded-sm border border-line bg-bg-primary p-0.5 text-xs">
          <button
            type="button"
          ...
          >
            <TreeStructure size={14} /> Model explorer
          </button>
          <button
            type="button"
        ...
        <button
          type="button"
          onClick={copyJson}
          className="flex items-center gap-1.5 rounded-sm border border-line bg-bg-secondary px-2.5 py-1 text-xs text-text-secondary transition hover:border-line-hover hover:text-text-primary"
        >
      ...
      {/* Model Rationale */}
      {model.rationale && (
        <div className="rounded-sm border border-line bg-bg-primary p-2.5 text-xs text-text-secondary">
          <div className="mb-1 flex items-center gap-1 font-semibold text-text-primary">
            <Sparkle size={13} weight="fill" className="text-accent-orange" /> Architectural rationale
          </div>
      ...
      {viewMode === 'tree' && (
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search model entities, slices, or properties…"
            className="w-full rounded-sm border border-line bg-bg-primary py-1.5 pr-3 pl-7 text-xs text-text-primary placeholder:text-text-muted focus:border-line-active focus:outline-none"
          />
        </div>
      )}
      ...
      {viewMode === 'tree' ? (
        <div className="rounded-sm border border-line bg-bg-primary p-3">
          {filteredCsm && Object.keys(filteredCsm).length > 0 ? (
            ...
            <p className="text-xs text-text-muted">No matching nodes found.</p>
          )}
        </div>
      ) : (
        <pre className="max-h-96 overflow-auto rounded-sm border border-line bg-bg-primary p-3 font-mono text-[11px] leading-relaxed text-text-secondary">
